...
为了有效地使用全局内存，理解 CUDA 编程模型中的翘曲概念非常重要，到目前为止，我们一直忽略了这一点。翘曲是 SMs 中线程调度/执行的一个单元。一旦一个块被分配给一个 SM，它就被分成一个称为**Warp**的 32 线程单元。这是 CUDA 编程中的基本执行单元。（1Block = 4Warp，1Warp = 32thread）
